...
function Swatch({ className, label, hex }: { className: string; label: string; hex?: string }) {
  return (
    <div>
      ...
      {hex ? <div className='text-label text-ink/70 tabular-nums'>{hex}</div> : null}
    </div>
  );
}
...
export const Color: StoryObj = {
  render: () => (
    <div>
      ...
      <div className='grid grid-cols-9 gap-1'>
        {ACCENT_RAMP.map(([step, className]) => (
          <div key={step}>
            ...
            <div className='mt-1 text-label text-ink/70 tabular-nums'>{step}</div>
          </div>
        ))}
      </div>
      ...
      <p className='mt-4 max-w-[680px] text-ui-sm text-ink/70'>
...
export const Typography: StoryObj = {
  render: () => (
    <div className='grid max-w-[900px] grid-cols-[220px_1fr] items-baseline gap-y-3'>
      <span className='text-label text-ink/70'>display · 400 · 64px</span>
      <span className='font-normal font-heading text-display tabular-nums'>$10,414.68</span>
      <span className='text-label text-ink/70'>h1 · 600 · 42px</span>
      <span className='font-semibold font-heading text-h1'>Expenses by category</span>
      <span className='text-label text-ink/70'>h2 · 600 · 32px</span>
      <span className='font-semibold font-heading text-h2'>Transactions</span>
      <span className='text-label text-ink/70'>h6 kicker · 13px caps</span>
      <Kicker>Budget projection</Kicker>
      <span className='text-label text-ink/70'>body · Lora · 15px</span>
      <span className='text-body'>
        A smaller, well-reasoned slice is worth more than a broad, shallow one.
      </span>
      <span className='text-label text-ink/70'>figures · tabular</span>
...
export const SpacingAndElevation: StoryObj = {
  ...
  render: () => (
    <div>
      <p className='mb-4 max-w-[680px] text-ui-sm text-ink/70'>
      ...
      <div className='flex flex-wrap items-end gap-4'>
        {[1, 2, 3, 4, 6, 8].map((step) => (
          <div key={step}>
            ...
            <div className='mt-1 text-label text-ink/70 tabular-nums'>
        ...
        {RADII.map(([size, className]) => (
          <div key={size}>
            ...
            <div className='mt-1 text-label text-ink/70'>radius {size}</div>
          </div>
        ))}
        <div className='w-6' />
        {SHADOWS.map(([size, className]) => (
          <div key={size}>
            ...
            <div className='mt-1 text-label text-ink/70'>shadow {size}</div>
          </div>
        ))}
      </div>
    </div>
