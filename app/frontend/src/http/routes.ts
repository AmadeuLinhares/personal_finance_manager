/**
 * Every URL the client knows, in one place.
 *
 * The API lives in this repo, so the paths sit next to the types they return
 * instead of coming from a published contracts package. Query strings are never
 * built here — that is `formatQueryParams`' job, so a route stays a route.
 */
export const routes = {
  meta: '/meta',
